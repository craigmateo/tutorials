...
def main():
    ...
    if validate_length(user_input):
        ...
        raise ValueError(f"Input length is too long. Must be under {MAX_INPUT_LENGTH}. Submitted input is {INPUT_LENGTH}")
...
def generate_branding_snippet(prompt: str) ->  str:
    ...
    response = openai.Completion.create(
        engine="text-davinci-001", prompt=enriched_prompt, temperature=0, max_tokens=32
        )

    # Extract output text and strip whitespace    
